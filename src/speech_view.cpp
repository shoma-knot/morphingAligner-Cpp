#include "speech_view.hpp"

#include <algorithm>
#include <cstdio>
#include <string>

#include <implot.h>

#include "app.hpp"

namespace {

// 単語/音素の区間の色。隣り合う区間を見分けられるよう、偶数/奇数番目で明るさを変える。
constexpr ImU32 kWordFill[2]  = { IM_COL32(70, 110, 170, 120), IM_COL32(90, 135, 200, 120) };
constexpr ImU32 kPhoneFill[2] = { IM_COL32(55, 135, 105, 120), IM_COL32(75, 160, 125, 120) };
constexpr ImU32 kUnknownFill  = IM_COL32(200, 120, 40, 150);    // spn（辞書にない語＝発話雑音扱い）
constexpr ImU32 kBoundaryCol  = IM_COL32(230, 230, 230, 150);
constexpr ImU32 kHoverCol     = IM_COL32(255, 255, 255, 230);
constexpr ImU32 kTextCol      = IM_COL32(255, 255, 255, 255);

// 無音を表すラベル（MFA は語ティアで "<eps>"、音素ティアで "sil" を出す）。
bool is_silence(const std::string& l) {
    return l.empty() || l == "<eps>" || l == "sil" || l == "sp";
}

// ティア名の表示（MFA の既定名だけ日本語にする）。
const char* tier_label(const std::string& name) {
    if (name == "words") return "単語";
    if (name == "phones") return "音素";
    return name.c_str();
}

constexpr int kRows = 2;    // 単語・音素（base/target で高さを揃えるため段数は固定）

}    // namespace

ImVec4 formant_color(int k) {
    // viridis の背景とアンカー（橙）の両方から見分けられる色。F1 から順に白・水色・桃・灰。
    static const ImVec4 kCols[] = {
        { 1.00f, 1.00f, 1.00f, 1.0f },
        { 0.35f, 0.90f, 1.00f, 1.0f },
        { 1.00f, 0.55f, 0.95f, 1.0f },
        { 0.75f, 0.75f, 0.75f, 1.0f },
    };
    constexpr int n = static_cast<int>(sizeof kCols / sizeof kCols[0]);
    return kCols[((k % n) + n) % n];
}

void draw_formants(const Formants& f) {
    for (int k = 0; k < static_cast<int>(f.tracks.size()); ++k) {
        const FormantTrack& tr = f.tracks[static_cast<std::size_t>(k)];
        if (tr.t.empty()) continue;
        ImPlotSpec spec;
        spec.Marker          = ImPlotMarker_Circle;
        spec.MarkerSize      = 1.5f;
        spec.MarkerFillColor = formant_color(k);
        spec.MarkerLineColor = formant_color(k);
        spec.Flags           = ImPlotItemFlags_NoFit;    // 軸の自動フィットに関与させない
        char label[16];
        std::snprintf(label, sizeof label, "##F%d", k + 1);
        ImPlot::PlotScatter(label, tr.t.data(), tr.erb.data(), static_cast<int>(tr.t.size()), spec);
    }
}

float segmentation_plot_height() {
    return kRows * (ImGui::GetTextLineHeight() + 8.0f) + 2.0f * ImPlot::GetStyle().PlotPadding.y + 2.0f;
}

void draw_segmentation(Track& tr, float width, float height) {
    const Segmentation& seg = tr.segmentation;
    const double        dur = tr.spec.duration;

    ImGui::PushID("segmentation");
    // 軸の装飾・メニュー・範囲選択は不要。時間軸のズーム/パン（ホイール・中ドラッグ）は
    // リンクしているのでスペクトログラムと連動する。
    if (!ImPlot::BeginPlot("##seg", ImVec2(width, height), ImPlotFlags_CanvasOnly)) {
        ImGui::PopID();
        return;
    }
    ImPlot::SetupAxes(nullptr, nullptr, ImPlotAxisFlags_NoDecorations,
                      ImPlotAxisFlags_Lock | ImPlotAxisFlags_NoGridLines | ImPlotAxisFlags_NoTickMarks);
    ImPlot::SetupAxisLinks(ImAxis_X1, &tr.view_x0, &tr.view_x1);
    ImPlot::SetupAxisLimitsConstraints(ImAxis_X1, 0, dur);
    ImPlot::SetupAxisLimits(ImAxis_Y1, 0, kRows, ImPlotCond_Always);

    // 段 k（上から）の中央に Y 目盛りとしてティア名を出す。結果が無くても同じ名前を出して
    // おき、実行前後でプロット領域の幅（＝スペクトログラムとの揃え）が変わらないようにする。
    const int   n_tiers         = std::min(kRows, static_cast<int>(seg.tiers.size()));
    double      tick_pos[kRows] = {};
    const char* tick_lab[kRows] = { "単語", "音素" };
    for (int k = 0; k < kRows; ++k) {
        tick_pos[k] = kRows - k - 0.5;
        if (k < n_tiers) tick_lab[k] = tier_label(seg.tiers[static_cast<std::size_t>(k)].name);
    }
    ImPlot::SetupAxisTicks(ImAxis_Y1, tick_pos, kRows, tick_lab, false);

    const ImPlotRect lim = ImPlot::GetPlotLimits();
    ImDrawList*      dl  = ImPlot::GetPlotDrawList();

    if (n_tiers == 0) {
        const char*  msg = tr.align_busy ? "音素セグメンテーションを実行中..." : "音素セグメンテーション未実行";
        const ImVec2 ts  = ImGui::CalcTextSize(msg);
        const ImVec2 p   = ImPlot::GetPlotPos();
        const ImVec2 s   = ImPlot::GetPlotSize();
        dl->AddText(ImVec2(p.x + (s.x - ts.x) * 0.5f, p.y + (s.y - ts.y) * 0.5f),
                    ImGui::GetColorU32(ImGuiCol_TextDisabled), msg);
        ImPlot::EndPlot();
        ImGui::PopID();
        return;
    }

    // カーソル下の区間（ツールチップと枠の強調用）。
    const bool         hovered = ImPlot::IsPlotHovered();
    const ImPlotPoint  mouse   = ImPlot::GetPlotMousePos();
    const SegInterval* hover   = nullptr;
    int                hover_k = -1;

    ImPlot::PushPlotClipRect();
    for (int k = 0; k < n_tiers; ++k) {
        const SegTier& tier     = seg.tiers[static_cast<std::size_t>(k)];
        const double   y_top    = kRows - k;
        const double   y_bottom = kRows - k - 1;
        const ImU32*   fill     = k == 0 ? kWordFill : kPhoneFill;
        for (std::size_t i = 0; i < tier.intervals.size(); ++i) {
            const SegInterval& iv = tier.intervals[i];
            if (iv.end < lim.X.Min || iv.start > lim.X.Max) continue;    // 表示範囲外
            const ImVec2 a = ImPlot::PlotToPixels(iv.start, y_top);
            const ImVec2 b = ImPlot::PlotToPixels(iv.end, y_bottom);

            if (!is_silence(iv.label))
                dl->AddRectFilled(a, b, iv.label == "spn" ? kUnknownFill : fill[i % 2]);
            dl->AddLine(ImVec2(a.x, a.y), ImVec2(a.x, b.y), kBoundaryCol);
            dl->AddLine(ImVec2(b.x, a.y), ImVec2(b.x, b.y), kBoundaryCol);

            // ラベルは区間に収まるときだけ出す（収まらないものはホバーで読む）。
            if (!is_silence(iv.label)) {
                const ImVec2 ts = ImGui::CalcTextSize(iv.label.c_str());
                if (ts.x + 4.0f <= b.x - a.x)
                    dl->AddText(ImVec2((a.x + b.x - ts.x) * 0.5f, (a.y + b.y - ts.y) * 0.5f), kTextCol,
                                iv.label.c_str());
            }

            if (hovered && mouse.y <= y_top && mouse.y > y_bottom && mouse.x >= iv.start && mouse.x < iv.end) {
                hover   = &iv;
                hover_k = k;
            }
        }
        // 段の区切り線。
        const float y = ImPlot::PlotToPixels(0.0, y_bottom).y;
        dl->AddLine(ImVec2(ImPlot::GetPlotPos().x, y),
                    ImVec2(ImPlot::GetPlotPos().x + ImPlot::GetPlotSize().x, y), kBoundaryCol);
    }
    if (hover != nullptr)
        dl->AddRect(ImPlot::PlotToPixels(hover->start, kRows - hover_k),
                    ImPlot::PlotToPixels(hover->end, kRows - hover_k - 1), kHoverCol, 0.0f, 0, 2.0f);
    ImPlot::PopPlotClipRect();

    if (hover != nullptr) {
        ImGui::BeginTooltip();
        ImGui::TextUnformatted(is_silence(hover->label) ? "（無音）" : hover->label.c_str());
        ImGui::Text("%.3f - %.3f s（%.0f ms）", hover->start, hover->end, (hover->end - hover->start) * 1000.0);
        ImGui::EndTooltip();
    }

    ImPlot::EndPlot();
    ImGui::PopID();
}
