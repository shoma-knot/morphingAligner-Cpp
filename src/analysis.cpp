...
Spectrogram analyze_file(const std::string& path) {
    ...
    if (x_length == 0) throw std::runtime_error("analyze_file: empty signal");
    ...
    Harvest(x.data(), x_length, fs, &h_opt, temporal_positions.data(), f0.data());
    ...
    std::vector<std::vector<double>> storage(f0_length, std::vector<double>(num_bins));
    std::vector<double*>             spectrogram(f0_length);
    for (int t = 0; t < f0_length; ++t)
        spectrogram[t] = storage[t].data();

    CheapTrick(x.data(), x_length, fs, temporal_positions.data(), f0.data(), f0_length, &c_opt, spectrogram.data());
    ...
    for (int t = 0; t < f0_length; ++t) {
        for (int b = 0; b < num_bins; ++b) {
            ...
            out.values[static_cast<std::size_t>(row) * f0_length + t] = static_cast<float>(db);
