...
**異なっている点（音への影響が大きい順）**
1. **周波数軸の空間**: MATLAB は **log 周波数**でワープ（アンカーも幾何平均 `exp((1-fx)logbf+fx logtf)`）。
   現状は**線形周波数**（`(1-fx)bf+fx tf`）。フォルマント等のスケーリングは log が自然で、
   影響が大きい。→ 合わせるなら `build_freq_warp` を log 周波数化。
2. **sp/ap の補間領域**: MATLAB は log スペクトログラムを時間・周波数とも interp1（log 領域で補間）。
   現状は線形パワーを bilinear サンプルしてから log 合成（サンプリングが線形領域）。
3. **非周期性(ap)の合成領域**: MATLAB は **log 領域**で補間・合成（clamp `[1e-5,1]`→log→重み→exp）。
   現状は**線形**（`(1-ap)ab+ap at`, clamp `[0,1]`）。
4. **F0**: (a) voicing 閾値 MATLAB=**0.99**（ほぼ全ソース有声を要求）／現状=**0.5**（優勢側が有声なら有声）。
   現状の方が有声フレームが増える。(b) 時間補間 MATLAB=**線形**(log fo)／現状=**最近傍**。
