...
- **アンカー打ち**（次の主タスク）: 各スペクトログラム上のクリック位置に時間/周波数アンカーを打ち、
  base↔target の対応を取れるようにする。
  - `ImPlot::GetPlotMousePos` / `PlotToPixels` でプロット座標⇔ピクセル座標を変換可能。
  - 対応線（パネル間の結線）やマーカーの描画方式は要相談。
